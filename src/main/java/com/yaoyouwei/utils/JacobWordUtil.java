 ...
 * JacobTest.java
 * Create on 2017年7月7日
 * Copyright 2017 Forwave All Rights Reserved
 */
package com.yaoyouwei.utils;

/**
 * @Author   yaoyouwei
 * @DateTime 2017年7月7日 下午3:30:48
 * @Version 2.0
 * @Description
 */
...
myDoc.Bookmarks("\Page").Range.Copy
...
pageCount = Selection.Information(wdNumberOfPagesInDocument) '获取新文档得文档总页数

'选中一页时,光标会停在下一页的起始位置,复制就会会多出一个字符占据了一个空页面(图片结尾则不会)
'当出现空页面时,删除这个多出的字符即可删除空页面
If pageCount > 1 Then
'wdStory=6 wdCharacter=1
Selection.EndKey Unit:=wdStory
Selection.TypeBackspace
Selection.Delete Unit:=wdCharacter, Count:=1
End If
aDoc.SaveAs FileName:="d:\" & CInt(i) & ".doc"
aDoc.Close
Next
...
*/
    ...
    public static void main(String[] args) {
    	...
    	ActiveXComponent wordApp1 = new ActiveXComponent("Word.Application");
    	//System.out.println(wordApp.getPropertyAsString("UserName"));
    	Dispatch wordDispatch = (Dispatch) wordApp.getObject();
    	//操作word过程可见
    	Dispatch.put(wordDispatch, "Visible", new Variant(visible)); 
    	//获取所有打开的word窗口
    	Dispatch documents = wordApp.getProperty("Documents").toDispatch();
    	//打开一个文档
    	Dispatch document = Dispatch.call(documents, "Open", "D://test.doc").toDispatch();
    	//设置当前文档的视图为页面 (视图->页面)
    	...
    	//重新编排页码
    	Dispatch.call(document, "Repaginate");
    	//获取文档选择区域或位置
    	Dispatch selection = Dispatch.get(wordApp, "Selection").toDispatch();
    	
    	/*Dispatch totalPageNumDispatch = Dispatch.call(document, "BuiltInDocumentProperties",new Variant(JacobConstant.wdPropertyPages)).toDispatch();
    	int totalPageNum = Dispatch.get(totalPageNumDispatch,"Value").getInt();*/
    	...
    	for(int i = 1 ;i<=totalPageNum;i++){
    		...
    		//光标移动到文档某一页的开始
    		 ...
    		 Dispatch.call(toSaveDocument, "SaveAs", new Variant("D://java_" + (i) + ".doc")); // 保存一个新文档
